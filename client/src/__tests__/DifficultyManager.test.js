...
describe('DifficultyManager', () => {
  ...
  it('should return MEDIUM for side piece when both sides are present', () => {
    const layerPieces = [{ present: true }, { present: true }, { present: true }]
    const layers = createMockLayers(1, layerPieces)
    expect(DifficultyManager.getDifficulty(0, 1, layers)).toBe(GAME.DIFFICULTY.MEDIUM)
    expect(DifficultyManager.getDifficulty(2, 1, layers)).toBe(GAME.DIFFICULTY.MEDIUM)
  })
  ...
  it('should return HARD for side piece when the other side is already extracted', () => {
    ...
    expect(DifficultyManager.getDifficulty(2, 1, layersLeft)).toBe(GAME.DIFFICULTY.HARD)

    const rightMissing = [{ present: true }, { present: true }, { present: false }]
    const layersRight = createMockLayers(1, rightMissing)
    expect(DifficultyManager.getDifficulty(0, 1, layersRight)).toBe(GAME.DIFFICULTY.HARD)
  })

  it('should handle middle piece when sides are missing', () => {
    const sidesMissing = [{ present: false }, { present: true }, { present: false }]
    const layers = createMockLayers(1, sidesMissing)
    expect(DifficultyManager.getDifficulty(1, 1, layers)).toBe(GAME.DIFFICULTY.HARD)
  })
})
